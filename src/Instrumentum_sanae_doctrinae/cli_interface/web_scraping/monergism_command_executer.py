...
def monegism_scrap_general_information(root_folder,browse_by_type:str,
                                       target_name:str,download_batch_size:int,
                                       overwrite_log):
    """_summary_
    ...
    if target_name.lower() == "all":
        ...
        if target_name in topic_dict:
            ...
            #print(ob.__dict__)
            asyncio.run(ob.print_download_informations(True))
            asyncio.run(ob.download_from_element_list([topic_dict[target_name]],1))
            ...
            #print(ob.__dict__)
            asyncio.run(ob.print_download_informations(True))
            asyncio.run(ob.download_from_element_list([speaker_dict[target_name]],1))
            
        if target_name in scripture_dict:
            ob = mn_scrap_general_information.MonergismScrapGeneralInformation_ALL(
            root_folder = root_folder,
            browse_by_type = browse_by_type,
            overwrite_log = overwrite_log
            ...
            #print(ob.__dict__)
            asyncio.run(ob.print_download_informations(True))
            asyncio.run(ob.download_from_element_list([scripture_dict[target_name]],1))
...
@click.pass_context
def monergism_scrap_general_information_command(context:click.Context,browse_by_type:str,
